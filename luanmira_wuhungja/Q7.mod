...
param machine_capacity_min{m in M} := 13440; # time capacity: machine minutes available in February
...
maximize Profit:
  ...
  - (sum{m in M, p in P} mach_op_min[m] *(proc_time[p,m]/60) * x[p]) #cost to make shoes using machine. per second
  - (labour_cost * sum{m in M, p in P}(proc_time[p,m]/3600) * x[p]) #labour cost
  - (sum{r in R, p in P} rm_cost[r] * bom[p,r] * x[p]) #cost to buy raw materials
...
subject to Machine_Capacity{m in M}:
    sum{p in P} (proc_time[p,m]/60) * x[p] <= machine_capacity_min[m]; #time has to be less than 12 hours a day 28 days a month, averaged out
